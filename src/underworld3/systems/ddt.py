...
class SemiLagrangian(uw_object):
    ...
    def _advect_history_PARKED(self, dt, evalf=False, verbose=False):
        ...
        Each psi_star[i] is sampled at positions traced back by Δt along
        the velocity characteristics, and the result is written back in place.

        Used by VE_Stokes for explicit stress history management where the
        actual stress is stored in psi_star[0] after each solve. This method
        advects those stored values to upstream positions before the next solve.

        Parameters
        ----------
        dt : float or sympy expression
            Timestep for characteristic tracing.
        evalf : bool
        ...
        # --- Coordinate setup (shared with update_pre_solve) ---
        from underworld3.utilities.unit_aware_array import UnitAwareArray
        ...
        cellid = self.mesh.get_closest_cells(psi_star_0_coords_nd)
        centroid_coords = self.mesh._centroids[cellid]
        shift = 0.001
        node_coords_nd = (1.0 - shift) * psi_star_0_coords_nd + shift * centroid_coords

        # --- Unit handling for dt ---
        model = uw.get_default_model()
        coords_template = self.psi_star[0].coords
        has_units = hasattr(coords_template, "magnitude") or hasattr(coords_template, "_magnitude")

        if has_units:
            dt_for_calc = dt.to("second") if hasattr(dt, "to") else dt
        else:
            if hasattr(dt, "magnitude") or hasattr(dt, "value"):
                dt_nondim = uw.non_dimensionalise(dt, model)
                if hasattr(dt_nondim, "magnitude"):
        ...
        # --- Advect each history level (RK2 midpoint method) ---
        for i in range(self.order - 1, -1, -1):
            # Evaluate velocity at node positions
            v_result = uw.function.evaluate(self.V_fn, node_coords_nd)
            if isinstance(v_result, UnitAwareArray):
                v_at_node_pts = v_result[:, 0, :]
                if not isinstance(v_at_node_pts, UnitAwareArray):
                    v_at_node_pts = UnitAwareArray(v_at_node_pts, units=v_result.units)
            ...
            # Non-dimensionalize velocities if needed
            if not has_units and isinstance(v_at_node_pts, UnitAwareArray):
                v_nondim = uw.non_dimensionalise(v_at_node_pts, model)
                v_at_node_pts = np.array(v_nondim) if isinstance(v_nondim, UnitAwareArray) else v_nondim
            ...
            if not has_units and isinstance(v_at_mid_pts, UnitAwareArray):
                v_nondim = uw.non_dimensionalise(v_at_mid_pts, model)
                v_at_mid_pts = np.array(v_nondim) if isinstance(v_nondim, UnitAwareArray) else v_nondim
            ...
            value_at_end_points = uw.function.global_evaluate(
                expr_to_evaluate, end_pt_coords,
            )
